...
import com.yidianhulian.secrettalk.activity.ContextMenu;
...
import com.yidianhulian.secrettalk.activity.AlertDialog;
import com.yidianhulian.secrettalk.activity.BaiduMapActivity;
import com.yidianhulian.secrettalk.activity.ChatActivity;
import com.yidianhulian.secrettalk.activity.MsgDetailsActivity;
import com.yidianhulian.secrettalk.activity.ShowNormalFileActivity;
import com.yidianhulian.secrettalk.activity.ShowVideoActivity;
import com.yidianhulian.secrettalk.task.LoadImageTask;
import com.yidianhulian.secrettalk.task.LoadVideoImageTask;
import com.yidianhulian.secrettalk.utils.ImageUtils;
...
public class MessageAdapter extends BaseAdapter {
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			if (!message.getBooleanAttribute(Const.MESSAGE_ATTR_IS_VOICE_CALL,
					false))
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			if (message.getType() == EMMessage.Type.IMAGE) {
				try {
					holder.msg = (TextView) convertView.findViewById(R.id.msg_iv_sendPicture);
				...
				if (message.getBooleanAttribute(
						Const.MESSAGE_ATTR_IS_VOICE_CALL, false)) {
					holder.iv = (ImageView) convertView
				...
				try {
					holder.msg = (TextView) convertView.findViewById(R.id.msg_iv_voice);
					holder.iv = ((ImageView) convertView
	...
	private void handleTextMessage(EMMessage message, ViewHolder holder,
		...
		final Spannable span = SmileUtils
				.getSmiledText(context, txtBody.getMessage());
		// 设置内容
		if(message.direct == EMMessage.Direct.SEND){
			holder.tv.setText("发送中...");
		}else{
			holder.tv.setText("新消息，点击查看");
		}
		...
		holder.tv.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				if(msg.status == Status.SUCCESS && msg.direct == EMMessage.Direct.SEND){
					conversation.removeMessage(msg.getMsgId());
					refresh();
					Toast.makeText(activity, "消息已销毁", Toast.LENGTH_SHORT).show();
				}else{
					Bundle bundle = new Bundle();
					Intent intent = new Intent();
					bundle.putString("msg", span.toString());
					bundle.putInt("position", position);
					bundle.putInt("type", MsgDetailsActivity.MESSAGE_TYPE_TXT);
					intent.putExtras(bundle);
					intent.setClass(activity, MsgDetailsActivity.class);
		...
		if (message.direct == EMMessage.Direct.SEND) {
			...
			case SUCCESS: // 发送成功
				...
				holder.tv.setText("发送成功");
				// 发送成功删除 信息
				break;
			case FAIL: // 发送失败
				holder.pb.setVisibility(View.GONE);
				holder.staus_iv.setVisibility(View.VISIBLE);
				holder.tv.setText("发送失败");
				break;
			case INPROGRESS: // 发送中
...
//		holder.iv.setOnLongClickListener(new OnLongClickListener() {
		...
		if(message.direct == EMMessage.Direct.SEND){
			holder.msg.setText("发送中...");
		}else{
			holder.msg.setText("新消息，点击查看");
		}
		
//		holder.iv.setImageResource(R.drawable.default_image);
		holder.iv.setImageResource(R.drawable.picture);
		// 接收方向的消息
...
//				holder.iv.setImageResource(R.drawable.default_image);
				...
				if (imgBody.getLocalUrl() != null) {
					...
					showImageView(thumbnailPath, holder.iv, holder.msg, filePath,
							imgBody.getRemoteUrl(), message, position);
				}
			}
			return;
		}
		...
		if (filePath != null && new File(filePath).exists()) {
			showImageView(ImageUtils.getThumbnailImagePath(filePath),
					holder.iv,holder.msg, filePath, null, message, position);
		} else {
			showImageView(ImageUtils.getThumbnailImagePath(filePath),
					holder.iv,holder.msg, filePath, IMAGE_DIR, message, position);
		}
		...
		case SUCCESS:
			...
			holder.tv.setVisibility(View.GONE);
			holder.staus_iv.setVisibility(View.GONE);
			holder.msg.setText("发送成功");
			break;
		case FAIL:
			holder.pb.setVisibility(View.GONE);
			holder.tv.setVisibility(View.GONE);
			holder.staus_iv.setVisibility(View.VISIBLE);
			holder.msg.setText("发送失败");
			break;
		case INPROGRESS:
			holder.staus_iv.setVisibility(View.GONE);
			holder.pb.setVisibility(View.VISIBLE);
		...
		holder.msg.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(msg.status == Status.SUCCESS && msg.direct == EMMessage.Direct.SEND){
					...
					Bundle bundle = new Bundle();
					bundle.putInt("position", position);
					bundle.putInt("type", MsgDetailsActivity.MESSAGE_TYPE_VOICE);
...
//		} else {
//			if (message.direct == EMMessage.Direct.RECEIVE) {
//				holder.iv.setImageResource(R.drawable.sound_to);
//			} else {
//				holder.iv.setImageResource(R.drawable.sound_from);
//			}
//		}
...
//				refresh();
//				ChatActivity chat = (ChatActivity) activity;
//				chat.setListView(getCount());
			}
...
//											.getString(R.string.connect_failuer_toast),
//							0).show();
					Toast.makeText( activity, "发送消息失败", 0 ).show();
				}
	...
	public void delete_msg(final EMMessage message,final Activity activity){
			...
			public void run() {
				try {
					...
					activity.runOnUiThread(new Runnable() {
						...
						@Override
						public void run() {
							Toast toast = Toast.makeText(activity,"消息发送成功", Toast.LENGTH_LONG);
							toast.setGravity(Gravity.CENTER, 0, 0);
							toast.show();
//							Toast.makeText(activity, "消息自动销毁", 1).show();
							conversation.removeMessage(message.getMsgId());
							ChatActivity chatActivity = (ChatActivity) activity;
							chatActivity.setListView(getCount());
							refresh();
						}
					});
	...
	private boolean showImageView(final String thumbernailPath,
			final ImageView iv,final TextView show, final String localFullSizePath,
			String remoteDir, final EMMessage message,final int position) {
		// String imagename =
...
//			iv.setOnClickListener(new View.OnClickListener() {
			...
			new LoadImageTask().execute(thumbernailPath, localFullSizePath,
					remote, message.getChatType(), iv, activity, message, position, MessageAdapter.this, username, show);
			return true;
		}
